...
@section('title')
	<title>Jemput Zakat | Badan Amil Zakat Nasional (BAZNAS) Kota Bogor</title>
@stop
	...
	<div class="page-header">
		...
		<div class="breadcrumb-line breadcrumb-line-component">
			<ul class="breadcrumb">
				...
				<li class="active">Jemput Zakat</li>
			</ul>
		</div>
	</div>
@stop
...
function loadData(id)
{
  ...
  $('div#data').load(APP_URL+'/jemputzakat-data/'+id,function(){
    ...
    $('table#data-berita').DataTable({
		...
		$('.switch').on('switchChange.bootstrapSwitch', function (event, state) {
			...
			$.ajax({
					dataType: 'json',
					type:'POST',
					url: APP_URL+'/jemputzakat-status/'+iduser,
					data:{validasi : st},
					...
					// getPageData();
					var txt = "Status Jemput Zakat Berhasil Di Edit";
				...
				new PNotify({
						title: 'Informasi',
						text: 'Edit Status Jemput Zakat Gagal',
...
function hapus(id)
{
	...
	$('#hapusbutton').click(function(){
        ...
        $('#modal-hapus').modal('hide');
				    new PNotify({
		            title: 'Berhasil',
		            text: 'Data Jemput Zakat Berhasil Di Hapus',
		            addclass: 'alert bg-success alert-styled-right',
		            type: 'success'
		        });
						loadData(-1);
        ...
        // toastr.success('Item Created Successfully.', 'Success Alert', {timeOut: 5000});
			...
			new PNotify({
					title: 'Informasi',
					text: 'Hapus Data Jemput Zakat Gagal',
					addclass: 'alert alert-warning alert-styled-right',
					type: 'error'
			});
		});
      ...
      <ul class="fab-menu-inner">
				<li>
					<div data-fab-label="Tambah Berita">
						<a href="{{URL::to('/jemputzakat-form/-1')}}" class="btn btn-default btn-rounded btn-icon btn-float">
